...
pack :: Eq a => [a] -> [[a]]
pack (x : xs) = let (h, l) = span (== x) xs in (x : h) : pack l
pack []       = []

encode :: Eq a => [a] -> [(Int, a)]
encode = map count . group where count xs = (myLength xs, head xs)
...
dropEvery x c = dropEvery' x c 1
 where
  dropEvery' (x : xs) c i | c == i    = dropEvery' xs c 1
...
rotate :: [a] -> Int -> [a]
rotate xs 0 = xs
rotate xs i | i < 0     = rotate (last xs : init xs) (i + 1)
